"""
LLM.int8 量化示范程序
这个程序展示了如何使用bitsandbytes库将大语言模型压缩为8bit精度，减少内存占用
同时保持较好的模型性能，适合在普通GPU上运行较大的语言模型
"""
...
# 从transformers库导入量化配置工具，用于设置8bit量化参数
from transformers import BitsAndBytesConfig


def main():
    """主函数：演示如何将语言模型量化为8bit并使用它进行文本生成"""

    # 第一步：配置8bit量化参数
    # 创建量化配置对象，设置量化的具体参数
    bnb_config = BitsAndBytesConfig(
        load_in_8bit=True,  # 关键参数：开启8bit量化，这会将模型权重压缩为8bit精度
        llm_int8_threshold=6.0,  # 量化阈值：数值大于6.0的激活值会用更高精度计算
                                 # 这样可以避免极端数值导致的性能下降
        llm_int8_has_fp16_weight=False  # 不使用16bit权重，完全使用8bit量化
    )
    ...
    # 第三步：加载量化后的模型
    print("\n===== 🚀 开始加载8bit量化模型 =====")
    # 从Hugging Face下载模型，并应用我们之前设置的量化配置
    model = AutoModelForCausalLM.from_pretrained(
        model_id,  # 指定要加载的模型ID
        device_map="auto",  # 自动将模型分配到可用的设备（优先使用GPU）
        quantization_config=bnb_config,  # 应用8bit量化配置
        trust_remote_code=True,  # 信任模型中的自定义代码
        torch_dtype=torch.float16,  # 模型计算时使用16bit浮点数
    ...
    # 检查模型是否成功进行了8bit量化（应该显示True）
    print(f"✅ 是否8bit量化: {model.is_loaded_in_8bit}")
    # 查看当前模型占用的GPU显存大小（MB）
